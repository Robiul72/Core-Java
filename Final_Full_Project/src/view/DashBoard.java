...
public class DashBoard extends javax.swing.JFrame {
    ...
    public void addProductToStock(){
        sql = " insert into stock (productName, productQuantity) values (?,?)";
        
        try {
            ps=con.getCon().prepareStatement(sql);
            
            ps.setString(1, stName.getText().trim());
            ps.setFloat(2, 0.0f);
    ...
    // purchase Cash Receive
    public float getCashReceive() {
        float prActualPrice = getActualPrice();
    ...
    private void prQuantityFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_prQuantityFocusLost
        ...
        try {
            if (prUnitPrice.getText().trim().isEmpty()) {
                ...
                prTotalPrice.setText(getTotalPrice() + " ");
            } else {
                JOptionPane.showMessageDialog(rootPane, "Quantity can not be Empty");
